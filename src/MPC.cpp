...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, std::vector<double> &x_path, std::vector<double> &y_path) {
  bool ok = true;
  size_t i;
  ...
  // creates a 2 element double vector.
	...
	//for path display
	for (int i = 0; i < N; ++i) {
	    x_path.push_back(solution.x[x_start + i]);
	    y_path.push_back(solution.x[y_start + i]);
	  }
